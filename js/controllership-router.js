...
async function abrirModuloCompartilhado(chave,arquivo,tabId){if(!validarContexto(chave))return;try{marcarAtivo(chave);if(!prepararShellCompartilhado())throw new Error("shell-controladoria-ausente");await importar(chave,arquivo);abrirPagina("controladoria");esconderTabsInternas();requestAnimationFrame(()=>$(tabId)?.click())}catch(e){erroModulo(chave,e)}}
async function abrirTela(chave,arquivo){if(!validarContexto(chave))return;try{marcarAtivo(chave);const m=await importar(chave,arquivo);if(typeof m.abrir!=="function")throw new Error("modulo-sem-funcao-abrir");m.abrir()}catch(e){erroModulo(chave,e)}}
