...
const modulos=new Map();const MODULO_VERSAO="20260901consorciosmenu2";
...
function marcarAtivo(chave){
  ...
  if(chave==="consorcios"){
    ...
    $("menuConsorcios")?.classList.remove("ativo");
...
function garantirMenuConsorcios(){
  let menu=$("menuConsorcios");
  if(!menu){
    const contratos=$("menuContratos");
    const separador=document.querySelector(".sidebar-menu .menu-separador");
    if(!contratos&&!separador)return;
    menu=document.createElement("button");
    menu.id="menuConsorcios";
    menu.className="menu-item hidden";
    menu.dataset.pagina="consorcios";
    menu.type="button";
    menu.textContent="Consórcios";
    if(contratos)contratos.insertAdjacentElement("afterend",menu);else separador.before(menu);
  }
  const ok=podeAbrir("consorcios");
  menu.classList.toggle("hidden",!ok);
  if(!ok)menu.classList.remove("ativo");
  if(menu.dataset.sigConsorciosBound!=="1"){
    menu.dataset.sigConsorciosBound="1";
    menu.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();ACOES.consorcios?.()},true);
  }
}
function montar(){
  css();
  garantirMenuConsorcios();
  const menu=$("menuControladoria");if(!menu)return;
  let box=$("ctrlSubmenu");
  if(!box){
...
montar();window.addEventListener("sig:ready",()=>{modulos.clear();montar()});window.addEventListener("sig:page",e=>{if(e.detail?.pagina==="controladoria")esconderTabsInternas();aplicarPermissoesSubmenu();garantirMenuConsorcios()});