...
const modulos=new Map();const MODULO_VERSAO="20260924-sales-consolidated-city-company-v21";
...
function moduloAcao(modulo,acoes=[]){if(admin())return true;return acoes.some(a=>permite(modulo,a))}
function podeAbrir(chave){
  ...
  switch(chave){
    ...
    case"inadimplencia":return ctrlAcao("inadimplencia")||ctrlAcao("inadimplenciaEditar");
...
montar();window.addEventListener("sig:ready",()=>{modulos.clear();montar()});window.addEventListener("sig:page",e=>{if(e.detail?.pagina==="controladoria")esconderTabsInternas();aplicarPermissoesSubmenu();garantirMenusOperacionais()});