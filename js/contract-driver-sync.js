...
import { listarDocumentos, listarDocumentosEmpresa, empresasSelecionadasIds, grupoAtualId, periodoAno, emitirAlteracao } from "./shared.js";
import { normalizarClassificacaoDre } from "./dre-classification.js";
...
function mesIndice(iso){const s=String(iso||"");if(!/^\d{4}-\d{2}/.test(s))return null;return Number(s.slice(0,4))*12+Number(s.slice(5,7))-1}
function competenciaIndice(ano,mes0){return Number(ano)*12+Number(mes0)}
function vigenteMes(c,ano,mes0){const alvo=competenciaIndice(ano,mes0),ini=mesIndice(c.inicio),fim=mesIndice(c.fim);return(ini===null||alvo>=ini)&&(fim===null||alvo<=fim)}
function valorContratoMes(c,ano,mes0){
  if(!vigenteMes(c,ano,mes0))return 0;
  const base=Math.abs(n(c.valorMensal));if(!base)return 0;
  const regra=c.regraReajuste||{},tipo=regra.tipo||"sem_reajuste",pct=tipo==="sem_reajuste"?0:n(regra.percentualProjetado),periodicidade=Math.max(1,n(regra.periodicidadeMeses)||12),ini=mesIndice(c.inicio),alvo=competenciaIndice(ano,mes0),ciclos=ini===null?0:Math.max(0,Math.floor((alvo-ini)/periodicidade));
  return base*Math.pow(1+pct/100,ciclos);
}
function valoresContrato(c,ano){return Object.fromEntries(MESES.map((m,i)=>[m,valorContratoMes(c,ano,i)]))}
...
async function reconciliarPlanejamentoEmpresa(empresaId,cenario,{versaoForcada="",atualizarTela=false}={}){
  if(!empresaId||!podePlan(cenario))return{alterados:0,bloqueados:0};
  const ano=periodoAno(),colecaoLinhas=cenario==="budget"?"budgetLinhas":"forecastLinhas";
  const [contratos,detalhes,linhas]=await Promise.all([
    listarDocumentosEmpresa("contratos",empresaId),
    listarDocumentosEmpresa("planejamentoDetalhes",empresaId),
    listarDocumentosEmpresa(colecaoLinhas,empresaId)
  ]);
  const versao=versaoForcada||versaoSelecionada(cenario)||versaoRecente(linhas,ano)||versaoPadrao(cenario,ano);
  if(cenario==="budget"&&statusBudgetMeta(linhas,ano,versao)==="finalizado")return{alterados:0,bloqueados:1};

  const existentes=detalhes.filter(d=>d.cenario===cenario&&Number(d.exercicio)===Number(ano)&&d.versao===versao&&d.origem==="contrato");
  const porContrato=new Map(existentes.map(d=>[d.origemContratoId,d]));
  ...
  for(const c of contratos){
    ...
    const valores=valoresContrato(c,ano);if(!temValor(valores))continue;
    const k=chaveConta(c.contaGerencialId,c.centroCustoId),linha=linhaCanonica(linhas,{ano,versao,contaId:c.contaGerencialId,cc:c.centroCustoId});
    if(linha?.lancamentoFechado===true)continue;
    desejados.set(c.id,{contrato:c,k,valores});
  }
  const novosPorChave=new Map();
  for(const x of desejados.values())novosPorChave.set(x.k,soma(novosPorChave.get(x.k)||vazio(),x.valores));

  let alterados=0;
  for(const [contratoId,x] of desejados){
    const ant=porContrato.get(contratoId),ref=doc(db,"planejamentoDetalhes",ant?.id||detalheId(cenario,ano,versao,contratoId));
    const payload={grupoId:grupoAtualId(),empresaId,cenario,exercicio:ano,versao,contaId:x.contrato.contaGerencialId,centroCustoId:x.contrato.centroCustoId,origem:"contrato",origemContratoId:contratoId,descricao:descricaoContrato(x.contrato),valores:x.valores,status:"ativo",automatico:true,comentario:x.contrato.comentarioPlanejamento||"",atualizadoEm:serverTimestamp()};
    if(!ant){payload.criadoPor=state.usuario?.id||"";payload.criadoEm=serverTimestamp()}
    await setDoc(ref,payload,{merge:true});alterados++;
  }
  ...
  for(const k of chaves){
    const delta=sub(novosPorChave.get(k)||vazio(),antigosPorChave.get(k)||vazio());if(!temValor(delta))continue;
    const {cc,contaId}=parseChave(k),linha=linhaCanonica(linhas,{ano,versao,contaId,cc});if(linha?.lancamentoFechado===true)continue;
    const atual={...vazio(),...(linha?.valores||{})},proximo=soma(atual,delta),ref=doc(db,colecaoLinhas,linha?.id||linhaDriverId(cenario,ano,versao,cc,contaId));
    const payload={grupoId:grupoAtualId(),empresaId,contaId,centroCustoId:cc,exercicio:ano,versao,valores:proximo,status:linha?.status||"rascunho",lancamentoFechado:linha?.lancamentoFechado===true,atualizadoEm:serverTimestamp()};
    if(cenario==="forecast")payload.realizadoFechadoAte=linha?.realizadoFechadoAte??fechadoForecast();
    if(!linha){payload.criadoPor=state.usuario?.id||"";payload.criadoEm=serverTimestamp()}
    await setDoc(ref,payload,{merge:true});alterados++;
  }
...
async function reconciliarCaixaEmpresa(empresaId){
  ...
  const [contratos,lancamentos,plano]=await Promise.all([
    listarDocumentosEmpresa("contratos",empresaId),
    listarDocumentosEmpresa("fluxoCaixaLancamentos",empresaId),
    listarDocumentosEmpresa("planoContasGerencial",empresaId).catch(()=>[])
  ]);
  ...
  for(const c of contratos){
    if(c.fluxoCaixaAtivo!==true||!ativoContrato(c)||!c.contaGerencialId||!n(c.diaVencimento))continue;
    const conta=pmap.get(c.contaGerencialId)||null;
    for(let q=0;q<=18;q++){
      const {ano,mes0}=addMeses(inicio.ano,inicio.mes0,q),comp=`${ano}-${String(mes0+1).padStart(2,"0")}`;if(comp>fimComp||!vigenteMes(c,ano,mes0))continue;
      const valor=valorContratoMes(c,ano,mes0);if(!valor)continue;
      desejados.set(`${c.id}|${comp}`,{c,comp,data:dataMensal(ano,mes0+1,n(c.diaVencimento)),valor,conta});
    }
  }
  let alterados=0;
  for(const [k,x] of desejados){
    const ant=porId.get(k);if(ant&&!["provisao"].includes(ant.status))continue;
    const ref=doc(db,"fluxoCaixaLancamentos",ant?.id||caixaId(x.c.id,x.comp)),payload={grupoId:grupoAtualId(),empresaId,data:x.data,natureza:contaNaturezaCaixa(x.conta),status:"provisao",contaBancariaId:ant?.contaBancariaId||"",classe:contaClasseCaixa(x.conta),centroCustoId:x.c.centroCustoId||"",categoria:"Contrato",descricao:descricaoContrato(x.c),valor:Math.abs(x.valor),observacao:x.c.comentarioPlanejamento||"Provisão automática originada no módulo de Contratos.",origem:"contrato",origemContratoId:x.c.id,competencia:x.comp,automatico:true,atualizadoEm:serverTimestamp()};
    if(!ant){payload.criadoPor=state.usuario?.id||"";payload.criadoEm=serverTimestamp()}
    await setDoc(ref,payload,{merge:true});alterados++;
  }
...
function instalarValidacao(){
  document.addEventListener("submit",e=>{
    if(e.target?.id!=="formContrato")return;
    const plan=document.getElementById("contratoPlanejamentoAtivo")?.checked===true,caixa=document.getElementById("contratoCaixaAtivo")?.checked===true,cc=document.getElementById("contratoCentroCusto")?.value||"",dia=n(document.getElementById("contratoDiaVencimento")?.value),mensagem=document.getElementById("mensagemContrato");
    if(plan&&!cc){e.preventDefault();e.stopImmediatePropagation();if(mensagem)mensagem.textContent="Para usar o contrato no Budget/Forecast, selecione um centro de custo.";return}
    if(caixa&&(dia<1||dia>31)){e.preventDefault();e.stopImmediatePropagation();if(mensagem)mensagem.textContent="Para provisionar no caixa, informe o dia de pagamento entre 1 e 31."}
  },true);
}
...
window.addEventListener("sig:contract-driver-changed",()=>agendar(reconciliarTudo,80));
window.addEventListener("sig:data-changed",e=>{if(e.detail?.modulo==="contratos")agendar(reconciliarTudo,120)});
window.addEventListener("sig:periodo-changed",()=>agendar(async()=>{const p=document.querySelector(".pagina:not(.hidden)")?.id||"";if(p.includes("budget"))await reconciliarTela("budget");else if(p.includes("forecast"))await reconciliarTela("forecast")},180));
window.addEventListener("sig:page",e=>{const p=String(e.detail?.pagina||"");if(p.includes("budget"))agendar(()=>reconciliarTela("budget"),260);else if(p.includes("forecast"))agendar(()=>reconciliarTela("forecast"),260)});
document.addEventListener("change",e=>{if(e.target?.id==="pagina-ctrl-budget-v7-versao")agendar(()=>reconciliarTela("budget"),100);if(["pagina-ctrl-forecast-v5-versao","pagina-ctrl-forecast-v5-fechado"].includes(e.target?.id))agendar(()=>reconciliarTela("forecast"),100)});
document.addEventListener("click",e=>{if(e.target?.closest?.("#tabFluxoCaixa,#lazyFluxoCaixa,[data-fpa-tab='caixa']"))agendar(reconciliarCaixaAtual,220)});
window.addEventListener("sig:ready",()=>agendar(reconciliarTudo,500));
