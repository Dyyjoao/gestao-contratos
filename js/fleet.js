...
let veiculos=[],manutencoes=[],plano=[],motoristas=[],editVeiculoId=null,editManutId=null,busy=false,tabAtual="visao";
...
async function carregar(){if(!podeVer())return;criarPagina();garantirMenu();busy=true;try{const [v,m,mo]=await Promise.all([listarDocumentos("veiculos"),listarDocumentos("manutencoesFrota"),colaboradoresPorFuncao("MOTORISTA").catch(e=>{console.warn("Motoristas do RH indisponíveis",e);return[]})]);veiculos=v;manutencoes=m;motoristas=mo;await carregarPlano();preencherContas();preencherVeiculoSelects();preencherMotoristas();renderTudo()}catch(e){console.error(e);alert(mensagemErroDados(e,"a Gestão de Frota"))}finally{busy=false}}
