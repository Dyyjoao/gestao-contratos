...
function bind(){
  ...
  $("btnCancelarObrig")?.addEventListener("click",()=>{$("formObrigacaoBox")?.classList.remove("fleet-obrig-modal");fechar("formObrigacaoBox","formObrigacao","msgObrig")});
...
function novaObrigacao(veiculoId=""){if(!podeObrig())return alert("Seu perfil não pode gerir obrigações da frota.");const id=typeof veiculoId==="string"?veiculoId:"";trocarTab("obrigacoes");$("formObrigacao")?.reset();preencherVeiculoSelects();if(id&&$("obrigVeiculo"))$("obrigVeiculo").value=id;if($("obrigStatus"))$("obrigStatus").value="aberto";const box=$("formObrigacaoBox");if(!box)return alert("Formulário de obrigação indisponível. Atualize a página.");box.classList.remove("hidden");box.classList.add("fleet-obrig-modal");setTimeout(()=>$("obrigVeiculo")?.focus(),60)}
