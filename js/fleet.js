...
let veiculos=[],manutencoes=[],abastecimentos=[],custosDiesel=[],plano=[],motoristas=[],editVeiculoId=null,editManutId=null,busy=false,tabAtual="visao";
...
<section class="fleet-panel" data-fleet-panel="visao">
  ...
  <div class="fleet-card">
    ...
    <div class="tabela-container"><table class="tabela fleet-table"><thead><tr><th>Veículo</th><th>Status</th><th>KM</th><th>Próximo vencimento</th><th>Manutenção</th><th>Custo 12m</th><th>Custo período</th><th>Imobilizado</th></tr></thead><tbody id="frotaResumoVeiculos"></tbody></table></div>
  </div>
</section>
...
<section class="fleet-panel hidden" data-fleet-panel="veiculos">
  <section id="formVeiculoBox" class="form-card hidden"><div class="form-card-titulo"><div><h3 id="tituloFormVeiculo">Novo veículo</h3><p>Cadastre a ficha operacional e, quando autorizado, o vínculo com o Imobilizado.</p></div></div>
    <form id="formVeiculo"><div class="form-grid form-grid-3">
      ...
      <div class="campo"><label id="veiculoKmLabel" for="veiculoKm">Quilometragem inicial</label><input id="veiculoKm" type="number" min="0" step="1"><small id="veiculoKmAjuda">Informe o hodômetro no início do controle. Os abastecimentos atualizarão o KM atual.</small></div>
      ...
      <div class="campo"><label for="veiculoResponsavel">Motorista / condutor principal</label><select id="veiculoResponsavel"><option value="">Sem motorista vinculado</option></select><small>Lista vinda dos colaboradores do RH com função Motorista / Frota.</small></div>
...
function preencherMotoristas(valorAtual=""){
  const s=$("veiculoResponsavel");if(!s)return;
  const atual=String(valorAtual||s.value||"").trim();
  const arr=[...motoristas].sort((a,b)=>String(a.nome||"").localeCompare(String(b.nome||""),"pt-BR"));
  s.innerHTML='<option value="">Sem motorista vinculado</option>'+arr.map(p=>`<option value="${esc(p.nome)}">${esc(p.nome)}${p.cargoNome?` · ${esc(p.cargoNome)}`:""}</option>`).join("");
  if(atual&&![...s.options].some(o=>o.value===atual)){const o=new Option(`${atual} · vínculo anterior`,atual);s.add(o)}
  s.value=atual;
}
async function carregar(){if(!podeVer())return;criarPagina();garantirMenu();busy=true;try{const [v,m,a,custo,mo]=await Promise.all([listarDocumentos("veiculos"),listarDocumentos("manutencoesFrota"),listarDocumentos("abastecimentosFrota"),listarDocumentos("custosDiesel"),colaboradoresPorFuncao("MOTORISTA").catch(e=>{console.warn("Motoristas do RH indisponíveis",e);return[]})]);veiculos=v;manutencoes=m;abastecimentos=a;custosDiesel=custo;motoristas=mo;await carregarPlano();preencherContas();preencherVeiculoSelects();preencherMotoristas();renderTudo()}catch(e){console.error(e);alert(mensagemErroDados(e,"a Gestão de Frota"))}finally{busy=false}}
function novoVeiculo(){if(!podeCadastrar())return alert("Seu perfil não pode cadastrar veículos.");if(!contextoOkGravacao())return alert("Selecione apenas uma empresa no cabeçalho para cadastrar o veículo.");editVeiculoId=null;$("formVeiculo")?.reset();$("veiculoVidaUtil").value=60;$("veiculoStatus").value="ativo";if($("veiculoKmLabel"))$("veiculoKmLabel").textContent="Quilometragem inicial";if($("veiculoKmAjuda"))$("veiculoKmAjuda").textContent="Informe o hodômetro no início do controle. Os abastecimentos atualizarão o KM atual.";preencherEmpresaSelect($("veiculoEmpresa"),{valorAtual:empresaUnicaSelecionadaId()});preencherContas();preencherMotoristas("");$("tituloFormVeiculo").textContent="Novo veículo";$("formVeiculoBox").classList.remove("hidden");$("veiculoPlaca")?.focus();trocarTab("veiculos")}
function editarVeiculo(id){if(!podeEditar())return;const v=vById(id);if(!v)return;editVeiculoId=id;$("formVeiculo")?.reset();if($("veiculoKmLabel"))$("veiculoKmLabel").textContent="Quilometragem atual";if($("veiculoKmAjuda"))$("veiculoKmAjuda").textContent=`KM inicial do controle: ${n(v.quilometragemInicial??v.quilometragemAtual).toLocaleString("pt-BR")} km`;preencherEmpresaSelect($("veiculoEmpresa"),{valorAtual:v.empresaId});preencherContas();$("veiculoPlaca").value=v.placa||"";$("veiculoRenavam").value=v.renavam||"";$("veiculoMarca").value=v.marca||"";$("veiculoModelo").value=v.modelo||"";$("veiculoAno").value=v.anoModelo||"";$("veiculoStatus").value=v.status||"ativo";$("veiculoKm").value=n(v.quilometragemAtual);$("veiculoDataAquisicao").value=v.dataAquisicao||"";$("veiculoValorAquisicao").value=n(v.valorAquisicao);preencherMotoristas(v.responsavel||"");$("veiculoContaAtivo").value=v.contaAtivoId||"";$("veiculoContaAcum").value=v.contaDepreciacaoAcumuladaId||"";$("veiculoContaDespesa").value=v.contaDepreciacaoId||"";$("veiculoVidaUtil").value=n(v.vidaUtilMeses)||60;$("veiculoDisponivel").value=v.dataDisponivelUso||v.dataAquisicao||"";$("veiculoIntegrarPlanejamento").checked=v.integrarPlanejamento===true;$("veiculoObs").value=v.observacoes||"";$("tituloFormVeiculo").textContent=`Editar ${v.placa||"veículo"}`;$("formVeiculoBox").classList.remove("hidden");trocarTab("veiculos");$("formVeiculoBox").scrollIntoView({behavior:"smooth",block:"start"})}
function dadosVeiculo(){const atual=editVeiculoId?vById(editVeiculoId):null;return{empresaId:$("veiculoEmpresa").value,placa:String($("veiculoPlaca").value||"").toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,7),renavam:String($("veiculoRenavam").value||"").replace(/\D/g,"").slice(0,11),marca:$("veiculoMarca").value.trim(),modelo:$("veiculoModelo").value.trim(),anoModelo:Math.trunc(n($("veiculoAno").value))||null,status:$("veiculoStatus").value,quilometragemInicial:atual?.quilometragemInicial??Math.trunc(n($("veiculoKm").value)),quilometragemAtual:Math.trunc(n($("veiculoKm").value)),dataAquisicao:$("veiculoDataAquisicao").value||"",valorAquisicao:n($("veiculoValorAquisicao").value),responsavel:$("veiculoResponsavel").value.trim(),centroCustoDescricao:atual?.centroCustoDescricao||"",contaAtivoId:podeContabil()?$("veiculoContaAtivo").value:(atual?.contaAtivoId||""),contaDepreciacaoAcumuladaId:podeContabil()?$("veiculoContaAcum").value:(atual?.contaDepreciacaoAcumuladaId||""),contaDepreciacaoId:podeContabil()?$("veiculoContaDespesa").value:(atual?.contaDepreciacaoId||""),vidaUtilMeses:podeContabil()?Math.max(1,Math.trunc(n($("veiculoVidaUtil").value)||60)):(atual?.vidaUtilMeses||60),dataDisponivelUso:podeContabil()?($("veiculoDisponivel").value||""):(atual?.dataDisponivelUso||""),integrarPlanejamento:podeContabil()?$("veiculoIntegrarPlanejamento").checked:(atual?.integrarPlanejamento===true),observacoes:$("veiculoObs").value.trim(),obrigacoes:obrigacoes(atual),ultimaConsultaOficialEm:atual?.ultimaConsultaOficialEm||"",proximaConsultaOficialEm:atual?.proximaConsultaOficialEm||"",imobilizadoId:atual?.imobilizadoId||""}}
...
function novaObrigacao(veiculoId=""){if(!podeObrig())return alert("Seu perfil não pode gerir obrigações da frota.");const id=typeof veiculoId==="string"?veiculoId:"";$("formObrigacao")?.reset();preencherVeiculoSelects();if(id&&$("obrigVeiculo"))$("obrigVeiculo").value=id;if($("obrigStatus"))$("obrigStatus").value="aberto";trocarTab("obrigacoes");$("formObrigacaoBox")?.classList.remove("hidden");$("formObrigacaoBox")?.scrollIntoView({behavior:"smooth",block:"start"});setTimeout(()=>$("obrigVeiculo")?.focus(),60)}
