...
import { listarDocumentos, atualizarDocumento, empresaUnicaSelecionadaId, periodoAno } from "./shared.js";
...
const contaPermitida=c=>contaAnalitica(c)&&contaDre(c)&&contaAtivaNoExercicio(c,periodoAno());
...
function renderLista(){
  ...
  box.innerHTML=arr.length?arr.map(c=>`<label class="ccv2-item"><input data-ccv2-conta="${esc(c.id)}" type="checkbox" ${sel.has(c.id)?"checked":""}><strong>${esc(c.codigo||"")}</strong><span>${esc(c.nome||"")}</span></label>`).join(""):'<div style="padding:18px;color:#667085;font-size:11px">Nenhuma conta Analítica de Receita/Despesa ativa neste exercício.</div>';
}
