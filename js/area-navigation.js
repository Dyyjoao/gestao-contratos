...
const ITENS={
  ...
  descarte:{label:"Perdas",root:"menuDescarte",area:"operacao",modulo:"descarte",acoes:["visualizar","lancar","editar"]},
  ...
  ordensservico:{label:"Ordens de Serviço",root:"menuOrdensServico",area:"operacao",modulo:"ordensServico",acoes:["visualizar","solicitar","executar"]},
...
function permitido(item){if(admin())return true;if(item.area==="rh"&&item.modulo!=="rhColaboradores"&&!["visualizar","lancar","editar"].some(a=>permite("rhColaboradores",a)))return false;return item.acoes.some(acao=>permite(item.modulo,acao)||(item.area==="logistica"&&permite("palletInventario",acao))||(item.area==="rh"&&item.modulo==="rhColaboradores"&&["rhAusencias","rhHorasColaboradores"].some(m=>permite(m,acao)))||(item.area==="seguranca"&&permite("modtreinamentos",acao)))}
function css(){
...
function garantirOperacao(){criarArea({menuId:"menuOperacao",boxId:"operacaoSubmenu",label:"Indústria",antesDe:"menuComercial",chaves:["producao","descarte","ordensservico"]})}
function garantirComercial(){criarArea({menuId:"menuComercial",boxId:"comercialSubmenu",label:"Comercial",antesDe:"menuControladoria",chaves:["vendas","visitas","orcamentos","reclamacoes"]})}
...
function garantirManutencao(){const m=$("menuManutencao"),b=$("manutencaoSubmenu");m?.remove();b?.remove()}
function garantirPessoas(){criarArea({menuId:"menuRh",boxId:"rhSubmenu",label:"RH",antesDe:"menuControladoria",chaves:["rh","avaliacoesrh","acoesrh"]});criarArea({menuId:"menuSeguranca",boxId:"segurancaSubmenu",label:"Segurança",antesDe:"menuControladoria",chaves:["seguranca"]})}
...
function marcarAtivo(chave=""){document.querySelectorAll(".area-subitem").forEach(b=>b.classList.toggle("ativo",b.dataset.areaChave===chave));const operacao=$("menuOperacao"),comercial=$("menuComercial"),ctrl=$("menuControladoria");if(operacao)operacao.classList.toggle("ativo",["producao","descarte","ordensservico"].includes(chave));if(comercial)comercial.classList.toggle("ativo",["vendas","visitas","orcamentos"].includes(chave));if(ctrl&&ORDEM_CONTROLADORIA.includes(chave))ctrl.classList.add("ativo");else if(ctrl&&["vendas","visitas","orcamentos","producao","descarte","ordensservico"].includes(chave))ctrl.classList.remove("ativo")}
function chavePorPagina(pagina=""){const p=String(pagina||"").toLowerCase();if(p.includes("producao"))return"producao";if(p.includes("descarte"))return"descarte";if(p.includes("visita"))return"visitas";if(p.includes("orcamento"))return"orcamentos";if(p.includes("reclamaco"))return"reclamacoes";if(p.includes("pallet"))return"pallets";if(p.includes("ordensservico"))return"ordensservico";if(p==="rh-avaliacoes")return"avaliacoesrh";if(p==="rh-acoes")return"acoesrh";if(p==="rh")return"rh";if(p==="seguranca")return"seguranca";if(p.includes("contas-pagar"))return"contasPagar";if(p.includes("contrat"))return"contratos";if(p.includes("permut"))return"permutas";if(p.includes("consor"))return"consorcios";if(p.includes("venda"))return"vendas";return""}
