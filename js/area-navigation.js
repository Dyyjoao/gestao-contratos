...
function garantirOperacao(){criarArea({menuId:"menuOperacao",boxId:"operacaoSubmenu",label:"Operação",antesDe:"menuComercial",chaves:["producao"]})}
...
function marcarAtivo(chave=""){document.querySelectorAll(".area-subitem").forEach(b=>b.classList.toggle("ativo",b.dataset.areaChave===chave));const operacao=$("menuOperacao"),comercial=$("menuComercial"),ctrl=$("menuControladoria");if(operacao)operacao.classList.toggle("ativo",chave==="producao");if(comercial)comercial.classList.toggle("ativo",chave==="vendas");if(ctrl&&ORDEM_CONTROLADORIA.includes(chave))ctrl.classList.add("ativo");else if(ctrl&&["vendas","producao"].includes(chave))ctrl.classList.remove("ativo")}
function chavePorPagina(pagina=""){const p=String(pagina||"").toLowerCase();if(p.includes("producao"))return"producao";if(p.includes("contas-pagar"))return"contasPagar";if(p.includes("contrat"))return"contratos";if(p.includes("permut"))return"permutas";if(p.includes("consor"))return"consorcios";if(p.includes("venda"))return"vendas";return""}
