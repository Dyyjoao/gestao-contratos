...
const ITENS={
  ...
  frota:{label:"Gestão de Veículos",root:"menuFrota",area:"frota",modulo:"frota",acoes:["visualizar","cadastrar","editar","manutencao","obrigacoes"]},
  combustivel:{label:"Combustível e Diesel",root:"menuCombustivel",area:"frota",modulo:"combustivel",acoes:["visualizar","lancar","editar"]},
...
function garantirFrota(){criarArea({menuId:"menuAreaFrota",boxId:"frotaAreaSubmenu",label:"Frota",antesDe:"menuControladoria",chaves:["frota","combustivel"]})}
...
function chavePorPagina(pagina=""){const p=String(pagina||"").toLowerCase();if(p.includes("producao"))return"producao";if(p.includes("descarte"))return"descarte";if(p.includes("visita"))return"visitas";if(p.includes("orcamento"))return"orcamentos";if(p.includes("reclamaco"))return"reclamacoes";if(p==="combustivel")return"combustivel";if(p==="frota")return"frota";if(p.includes("pallet"))return"pallets";if(p.includes("ordensservico"))return"ordensservico";if(p==="rh")return"rh";if(p==="seguranca")return"seguranca";if(p.includes("contas-pagar"))return"contasPagar";if(p.includes("contrat"))return"contratos";if(p.includes("permut"))return"permutas";if(p.includes("consor"))return"consorcios";if(p.includes("venda"))return"vendas";return""}
function aplicar(){css();garantirOperacao();garantirComercial();garantirLogistica();garantirFrota();garantirManutencao();garantirPessoas();garantirControladoriaTransferidos();ocultarRaizes();marcarAtivo(chaveAtiva)}
