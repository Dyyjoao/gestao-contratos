...
import { $, esc, listarDocumentos, criarDocumento, atualizarDocumento, empresaUnicaSelecionadaId, emitirAlteracao } from './shared.js';
...
async function carregar(){
  ...
  decorar();
  if(!empresaId)return;
  const [cfgs,pessoas]=await Promise.all([
    listarDocumentos('configuracoesModulos').catch(e=>{console.warn('Base central de cargos indisponível',e);return[]}),
    listarDocumentos('rhColaboradores').catch(e=>{console.warn('Colaboradores indisponíveis para cargos',e);return[]})
  ]);
  base=cfgs.find(x=>x.modulo===MODULO&&x.empresaId===empresaId)||null;
...
function atualizarCampoCargo(){
  const select=$('rhCargoForm');if(!select)return;
  const valor=select.value||base?.vinculos?.[colaboradorAtualFormulario()?.id]||'';
  select.innerHTML=opcoesCargo(valor);select.value=valor;
}
function injetarCargoAdmissao(){
  const campos=$('rhCampos'),setor=$('rhSetorForm');if(!campos||!setor)return;
  let select=$('rhCargoForm');
  if(!select){
    const atual=colaboradorAtualFormulario(),cargoId=base?.vinculos?.[atual?.id]||'';
    const div=document.createElement('div');div.className='campo';div.id='rhCargoCampo';div.innerHTML=`<label for="rhCargoForm">Cargo</label><select id="rhCargoForm">${opcoesCargo(cargoId)}</select><small>Base central usada por Frota, Vendas e outros módulos.</small>`;
    setor.closest('.campo')?.insertAdjacentElement('afterend',div);select=$('rhCargoForm');
  }else atualizarCampoCargo();
...
function garantirPainel(){
  ...
  const aviso=$('rhAviso');if(aviso)aviso.insertAdjacentElement('afterend',box);else pagina.querySelector('.rh-module-head')?.insertAdjacentElement('afterend',box);
...
function garantirBotao(){
  ...
  const atualizar=$('rhAtualizar');if(atualizar&&atualizar.parentElement===acoes)acoes.insertBefore(b,atualizar);else acoes.prepend(b);
}
...
function agendar(){clearTimeout(timer);decorar();timer=setTimeout(()=>carregar(),80)}
function instalar(){if(observador)return;observador=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(decorar,30)});observador.observe(document.body,{childList:true,subtree:true});agendar()}
...
window.addEventListener('sig:page',e=>{if(e.detail?.pagina==='rh'){decorar();agendar()}});
instalar();
