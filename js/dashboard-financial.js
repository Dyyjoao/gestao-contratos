...
function versaoMaisRecente(arr,ano){const docs=arr.filter(x=>Number(x.exercicio)===Number(ano)&&x.versao);if(!docs.length)return"";const mapa=new Map();docs.forEach(x=>mapa.set(x.versao,Math.max(mapa.get(x.versao)||0,stamp(x))));return [...mapa.entries()].sort((a,b)=>b[1]-a[1]||String(b[0]).localeCompare(String(a[0]),"pt-BR"))[0]?.[0]||""}
...
function somaCenario({docs,plano,classes,ano,periodo,versao=""}){const pmap=new Map(plano.map(x=>[x.id,x])),cmap=new Map(classes.map(x=>[x.contaId,x.classificacao||"opex"])),out={receita:0,opex:0,capex:0,resultado:0};docs.filter(x=>Number(x.exercicio)===Number(ano)&&(!versao||x.versao===versao)).forEach(x=>{const conta=pmap.get(x.contaId);if(!conta)return;const v=valorPeriodo(x.valores,periodo),classe=cmap.get(x.contaId)||"opex";if(classe==="capex"){out.capex+=Math.abs(v);return}if(conta.natureza==="receita")out.receita+=v;else out.opex+=Math.abs(v)});out.resultado=out.receita-out.opex;return out}
function somaForecast({forecasts,realizados,plano,classes,ano,periodo,versao}){const pmap=new Map(plano.map(x=>[x.id,x])),cmap=new Map(classes.map(x=>[x.contaId,x.classificacao||"opex"])),fdocs=forecasts.filter(x=>Number(x.exercicio)===Number(ano)&&(!versao||x.versao===versao)),chaves=new Map(),out={receita:0,opex:0,capex:0,resultado:0};realizados.filter(x=>Number(x.exercicio)===Number(ano)).forEach(x=>chaves.set(`${x.empresaId||""}|${x.contaId}|${x.centroCustoId||""}`,{empresaId:x.empresaId||"",contaId:x.contaId,cc:x.centroCustoId||""}));fdocs.forEach(x=>chaves.set(`${x.empresaId||""}|${x.contaId}|${x.centroCustoId||""}`,{empresaId:x.empresaId||"",contaId:x.contaId,cc:x.centroCustoId||""}));chaves.forEach(k=>{const conta=pmap.get(k.contaId);if(!conta)return;const r=realizados.find(x=>(x.empresaId||"")===k.empresaId&&Number(x.exercicio)===Number(ano)&&x.contaId===k.contaId&&(x.centroCustoId||"")===k.cc),f=fdocs.find(x=>(x.empresaId||"")===k.empresaId&&x.contaId===k.contaId&&(x.centroCustoId||"")===k.cc),fechado=num(f?.realizadoFechadoAte),vals={};MESES.forEach((m,i)=>vals[m]=i<fechado?num(r?.valores?.[m]):num(f?.valores?.[m]));const v=valorPeriodo(vals,periodo),classe=cmap.get(k.contaId)||"opex";if(classe==="capex")out.capex+=Math.abs(v);else if(conta.natureza==="receita")out.receita+=v;else out.opex+=Math.abs(v)});out.resultado=out.receita-out.opex;return out}
...
function renderTabela(real,budget,forecast){const rows=[["Receita",real.receita,budget.receita,forecast.receita,false],["OPEX",real.opex,budget.opex,forecast.opex,true],["Resultado",real.resultado,budget.resultado,forecast.resultado,false,"resultado"],["CAPEX",real.capex,budget.capex,forecast.capex,true,"capex"]],tb=$("dashFinTabela");if(!tb)return;tb.innerHTML=rows.map(([nome,r,b,f,inv,classe])=>`<tr class="${classe||""}"><td>${esc(nome)}</td><td>${moeda(r)}</td><td>${moeda(b)}</td><td>${moeda(f)}</td><td class="dash-finance-var ${varClass(r,b,inv)}">${pctTxt(r,b)}</td></tr>`).join("")}
...
export async function carregar(){
  ...
  carregando=true;
  try{
    const [plano,classes,realizados,budgets,forecasts,contas,lancamentos]=await Promise.all([listarDocumentos("planoContasGerencial"),listarDocumentos("classificacoesContas"),listarDocumentos("realizadoMensal"),listarDocumentos("budgetLinhas"),listarDocumentos("forecastLinhas"),listarDocumentos("contasBancarias"),listarDocumentos("fluxoCaixaLancamentos")]);
    const bv=versaoMaisRecente(budgets,ano),fv=versaoMaisRecente(forecasts,ano),real=somaCenario({docs:realizados,plano,classes,ano,periodo}),budget=somaCenario({docs:budgets,plano,classes,ano,periodo,versao:bv}),forecast=somaForecast({forecasts,realizados,plano,classes,ano,periodo,versao:fv}),margem=real.receita?(real.resultado/Math.abs(real.receita))*100:0;
    set("dashFinReceita",moeda(real.receita));set("dashFinReceitaSub",`${pctTxt(real.receita,budget.receita)} vs Budget`);set("dashFinOpex",moeda(real.opex));set("dashFinOpexSub",`${pctTxt(real.opex,budget.opex)} vs Budget`);set("dashFinResultado",moeda(real.resultado));set("dashFinResultadoSub",`${pctTxt(real.resultado,budget.resultado)} vs Budget`);set("dashFinMargem",`${margem.toLocaleString("pt-BR",{maximumFractionDigits:1})}%`);set("dashFinMargemSub","Resultado ÷ Receita");set("dashFinCapex",moeda(real.capex));set("dashFinCapexSub",`${pctTxt(real.capex,budget.capex)} vs Budget`);set("dashFinForecast",moeda(forecast.resultado));set("dashFinForecastSub",fv?`Versão ${fv}`:"Sem Forecast salvo");set("dashFinLegenda",`${rotulo} · ${ano} · Budget ${bv||"não identificado"} · Forecast ${fv||"não identificado"}`);set("dashResultadoGerencial",moeda(real.resultado));set("dashVariacaoBudget",`${pctTxt(real.resultado,budget.resultado)} vs. budget`);renderTabela(real,budget,forecast);renderCaixa(contas,lancamentos);
