...
function renderRecebimentos(){
  ...
  tb.innerHTML=arr.map(r=>{const al=(Array.isArray(r.alocacoes)?r.alocacoes:[]).map(a=>`${a.parcelaId} · ${moeda(n(a.valor))}`).join(" + ");return`<tr><td>${dataBr(r.dataRecebimento)}</td><td><strong>${esc(r.pedido||"—")}</strong></td><td>${esc(r.clienteNome||r.cliente||"—")}</td><td><strong>${moeda(n(r.valor))}</strong></td><td>${esc(al||"—")}</td></tr>`}).join("")
}
function render(){
