...
function criarPagina(){if(pagina())return;css();const main=document.querySelector("main.conteudo");if(!main)return;const s=document.createElement("section");s.id="pagina-ctrl-inadimplencia-v1";s.className="pagina hidden";s.innerHTML=`
  ...
  <div class="inad-aging-title"><div><h3>Posição acumulada</h3><small>Estoque histórico até a data de referência</small></div></div>
  <div class="kpi-grid inad-kpis">
    ...
    <div class="kpi-card"><span>Recebido acumulado</span><strong id="inadRecebidoRef">—</strong><small>principal baixado até a referência</small></div>
  </div>

  <div class="inad-aging-title"><div><h3>Período selecionado</h3><small id="inadPeriodoResumo">—</small></div></div>
  <div class="kpi-grid inad-kpis">
    <div class="kpi-card" data-inad-filter="aberto" data-inad-scope="periodo"><span>Carteira do período</span><strong id="inadPeriodoCarteira">—</strong><small>saldo das parcelas do período</small></div>
    <div class="kpi-card" data-inad-filter="vencidos" data-inad-scope="periodo"><span>Valor vencido do período</span><strong id="inadPeriodoVencido">—</strong><small>saldo vencido do período</small></div>
    <div class="kpi-card"><span>Índice de inadimplência do período</span><strong id="inadPeriodoIndice">—</strong><small>vencido ÷ carteira do período</small></div>
    <div class="kpi-card"><span>Recebido no período</span><strong id="inadPeriodoRecebido">—</strong><small>principal baixado dentro do período</small></div>
  </div>
...
function calcular(){
  ...
  const lim=limitesPeriodoSelecionado(),arrPeriodo=arr.filter(({v,p})=>parcelaNoPeriodo(v,p)),ativosPeriodo=arrPeriodo.filter(({v,p})=>saldoParcela(v,p,ref)>0),carteiraPeriodo=ativosPeriodo.reduce((s,{v,p})=>s+saldoParcela(v,p,ref),0),faixasPeriodo={sem_vencimento:[],a_vencer:[],"1_30":[],"31_60":[],"61_90":[],"90_mais":[]};
  ativosPeriodo.forEach(x=>{const b=bucket(x.v,x.p,ref);if(faixasPeriodo[b])faixasPeriodo[b].push(x)});
  const somaPeriodo=k=>faixasPeriodo[k].reduce((s,{v,p})=>s+saldoParcela(v,p,ref),0),vencidosPeriodo=ativosPeriodo.filter(({v,p})=>["1_30","31_60","61_90","90_mais"].includes(bucket(v,p,ref))),vencidoPeriodo=vencidosPeriodo.reduce((s,{v,p})=>s+saldoParcela(v,p,ref),0);
  const recebidoPeriodo=recebimentos.filter(r=>{const d=String(r.dataRecebimento||"").slice(0,10);return d&&d>=lim.inicio&&d<=lim.fim&&d<=ref}).reduce((s,r)=>s+n(r.valorPrincipal||(Array.isArray(r.alocacoes)?r.alocacoes.reduce((a,x)=>a+n(x.valor),0):0)),0);
  return{carteira,valorVencido,faixas,soma,recebidoRef,faixasPeriodo,somaPeriodo,carteiraPeriodo,vencidoPeriodo,recebidoPeriodo}
}
...
function render(){
  ...
  $("inadPeriodoCarteira").textContent=moeda(c.carteiraPeriodo);$("inadPeriodoVencido").textContent=moeda(c.vencidoPeriodo);$("inadPeriodoIndice").textContent=pct(c.vencidoPeriodo,c.carteiraPeriodo);$("inadPeriodoRecebido").textContent=moeda(c.recebidoPeriodo);$("inadPeriodoResumo").textContent=`${periodoNome} · posição em ${dataBr(ref)}`;$("inadPeriodoTitulo").textContent=`${periodoNome} · posição em ${dataBr(ref)}`;
