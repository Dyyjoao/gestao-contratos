...
export function arvoreContas(contas,{grupoDre=null,estatistica=null}={}){
  const base=(contas||[]).filter(c=>c.status!=="inativo"&&(grupoDre===null||c.grupoDre===grupoDre)&&(estatistica===null||contaEstatistica(c)===estatistica));
  const ids=new Set(base.map(c=>c.id)),porPai=new Map();base.forEach(c=>{const pai=ids.has(c.contaPaiId)?c.contaPaiId:"";if(!porPai.has(pai))porPai.set(pai,[]);porPai.get(pai).push(c)});porPai.forEach(a=>a.sort(ordenaContas));
  const out=[];function visita(pai,nivel){for(const c of porPai.get(pai)||[]){out.push({conta:c,nivel});visita(c.id,nivel+1)}}visita("",0);return out;
}
