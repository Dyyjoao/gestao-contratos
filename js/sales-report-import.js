...
function render(){
  ...
  $("salesReportVendedores").innerHTML=vend.map(x=>`<tr><td><strong>${esc(x.codigo)}</strong></td><td>${esc(x.nome||"—")}</td><td>${x.rh?`<strong>${esc(x.rh.nome)}</strong><small>${esc(x.rh.cargoNome||"")}</small>`:'<span class="status-inativo">Código não vinculado no RH</span>'}</td><td>${x.cfg?`<span class="status-ativo">${n(x.cfg.comissaoPct).toLocaleString("pt-BR",{maximumFractionDigits:3})}% · recebido${x.cfg.configuracaoPendente?" · pendente":""}</span>`:x.rh?'<span class="status-inativo">Será criada automaticamente em 0% · revisar depois</span>':'<span class="status-inativo">Código não vinculado no RH</span>'}</td><td>${x.qtd}</td></tr>`).join("");
