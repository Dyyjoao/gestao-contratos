...
function montarIntegracao(){
  ...
  const card=document.createElement("div");card.className="campo-span-3 ap-contract-card";card.innerHTML=`
    <div class="ap-contract-info"><strong>Contas a Pagar</strong><small>Envie os vencimentos mensais deste contrato ao cockpit operacional. Defina também a conta bancária prevista. Esta integração não alimenta DRE, Budget ou Forecast.</small></div>
...
async function sincronizarEmpresa(empresaId){
  ...
  for(const [k,x] of desejados){
    ...
    const payload={grupoId:grupoAtualId(),empresaId,fornecedor:x.c.fornecedor||"Fornecedor",descricao:x.c.objeto||`Contrato ${x.c.numero||""}`,documento:x.c.numero||"",categoria:"Contrato",valor:Math.abs(n(x.c.valorMensal)),vencimento:x.vencimento,responsavel:x.c.responsavel||"",observacao:"Obrigação gerada automaticamente pelo módulo de Contratos.",origem:"contrato",origemContratoId:x.c.id,contratoReferencia:x.c.numero||x.c.fornecedor||"Contrato",competencia:x.competencia,contaBancariaId:x.c.contaBancariaIdContasPagar||"",contaBancariaNome:x.c.contaBancariaNomeContasPagar||"",status:reabrir?"aberto":ant?.status||"aberto",canceladoAutomatico:false,atualizadoEm:serverTimestamp()};
...
function instalarEventos(){
  ...
  window.addEventListener("sig:contract-driver-changed",e=>agendar(async()=>{const id=e.detail?.contratoId;if(!id||!salvamentoPendente)return;try{await atualizarDocumento("contratos",id,{contasPagarAtivo:salvamentoPendente.ativo,contaBancariaIdContasPagar:salvamentoPendente.contaId,contaBancariaNomeContasPagar:salvamentoPendente.contaNome});salvamentoPendente=null;await atualizarCache();const c=contratosCache.find(x=>x.id===id);if(c?.empresaId)await sincronizarEmpresa(c.empresaId)}catch(err){console.error("Falha ao vincular contrato ao Contas a Pagar",err)}},60));
