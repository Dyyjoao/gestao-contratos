...
import { $, esc, state, moeda, dataBr, hojeIso, listarDocumentos, atualizarDocumento, emitirAlteracao } from "./shared.js";
...
function garantirCampoConta(){
  ...
  if($("apContaBancariaPrevista"))return;
  ...
  const resp=$("apResponsavel")?.closest(".campo");if(resp)resp.after(campo);else grid.appendChild(campo);
  $("apEmpresa")?.addEventListener("change",()=>preencherContaLancamento(""));
}

function preencherContaLancamento(valor=""){
  const sel=$("apContaBancariaPrevista");if(!sel)return;const empresaId=$("apEmpresa")?.value||"";
  if(podeVincularBanco()){
    const html=opcoesConta(empresaId,valor);setHtml(sel,html);if([...sel.options].some(o=>o.value===valor))sel.value=valor;sel.disabled=false;
  }else{setHtml(sel,'<option value="">Definida pelo Administrador</option>');sel.disabled=true}
}

async function prepararFormulario(id=""){
  await carregarDados();garantirCampoConta();const x=dados.find(a=>a.id===id);preencherContaLancamento(contaPlanejadaId(x));
}
...
function garantirPeriodo(){
  ...
  [["apFiltroInicio","De"],["apFiltroFim","Até"]].forEach(([id,texto])=>{
    const input=$(id);if(!input||input.closest(".ap-periodo-campo"))return;const w=document.createElement("label");w.className="ap-periodo-campo";w.innerHTML=`<span>${texto}</span>`;input.before(w);w.appendChild(input);
  });
...
function garantirFiltroConta(){
  const filtros=document.querySelector("#pagina-contas-pagar .ap-filtros");if(!filtros)return;
  let sel=$("apFiltroBanco");if(!sel){sel=document.createElement("select");sel.id="apFiltroBanco";sel.title="Conta bancária";sel.addEventListener("change",aplicarFiltroConta);$("apFiltroOrigem")?.after(sel)}
  const atual=sel.value,mapa=new Map();contas.forEach(c=>mapa.set(c.id,contaNome(c)));dados.forEach(x=>{const id=contaEfetivaId(x),nome=x.status==="pago"?contaPagaNome(x):contaPlanejadaNome(x);if(id&&nome&&!mapa.has(id))mapa.set(id,nome)});
  const html='<option value="">Todas as contas</option><option value="__sem__">Sem conta vinculada</option>'+[...mapa.entries()].sort((a,b)=>a[1].localeCompare(b[1],"pt-BR")).map(([id,nome])=>`<option value="${esc(id)}">${esc(nome)}</option>`).join("");
  if(sel.innerHTML!==html){sel.innerHTML=html;if([...sel.options].some(o=>o.value===atual))sel.value=atual}
}
...
async function abrirBaixa(id){
  await carregarDados();garantirModal();const x=dados.find(a=>a.id===id);if(!x)return alert("Não foi possível localizar este compromisso.");
  ...
  if(podeVincularBanco()){
    const elegiveis=contas.filter(c=>c.empresaId===x.empresaId);let html='<option value="">Selecione a conta</option>'+elegiveis.map(c=>`<option value="${esc(c.id)}">${esc(contaNome(c))}</option>`).join("");if(previstaId&&!elegiveis.some(c=>c.id===previstaId))html+=`<option value="${esc(previstaId)}">${esc(previstaNome||"Conta vinculada")}</option>`;setHtml(sel,html);sel.value=previstaId||"";sel.disabled=false;
  }else{
    if(!previstaId)return alert("Este compromisso ainda não tem conta bancária vinculada. O Administrador precisa definir a conta antes da baixa.");setHtml(sel,`<option value="${esc(previstaId)}">${esc(previstaNome||"Conta vinculada")}</option>`);sel.value=previstaId;sel.disabled=true;
  }
  ...
  const m=$("apBaixaBackdrop");m.dataset.registroId=id;m.classList.remove("hidden");$("apBaixaConfirmar").onclick=confirmarBaixa;
}
...
async function confirmarBaixa(){
  ...
  const conta=contas.find(c=>c.id===contaId),nome=contaNome(conta)||($("apBaixaBanco")?.options?.[$("apBaixaBanco").selectedIndex]?.textContent||contaPlanejadaNome(x));
  try{$("apBaixaConfirmar").disabled=true;await atualizarDocumento("contasPagar",id,{status:"pago",dataPagamento,pagoPor:state.usuario?.id||"",contaBancariaIdPagamento:contaId,contaBancariaNomePagamento:nome});fecharBaixa();emitirAlteracao(MODULO)}catch(e){console.error(e);alert("Não foi possível marcar o pagamento.")}finally{if($("apBaixaConfirmar"))$("apBaixaConfirmar").disabled=false}
}
...
function aplicarFiltroConta(){
  ...
  tabela.querySelectorAll("tbody tr").forEach(row=>{const id=idRegistroDaLinha(row);if(!id)return;const x=dados.find(a=>a.id===id),idConta=contaEfetivaId(x),mostrar=!filtro||(filtro==="__sem__"?!idConta:idConta===filtro);row.style.display=mostrar?"":"none";row.dataset.apBancoOculto=mostrar?"0":"1";if(mostrar)visiveis++});
  const q=$("apQuantidade");if(q)q.textContent=filtro?`${visiveis} exibido(s) na conta selecionada · ${dados.length} registro(s) no contexto`:`${visiveis} exibido(s) · ${dados.length} registro(s) no contexto`;
}
...
function preparar(){instalarCss();garantirCampoConta();garantirPeriodo();garantirFiltroConta();garantirModal();decorarTabela()}
function agendarPreparar(){if(prepararAgendado)return;prepararAgendado=true;setTimeout(()=>{prepararAgendado=false;preparar()},0)}

const obs=new MutationObserver(agendarPreparar);obs.observe(document.body,{childList:true,subtree:true});
document.addEventListener("submit",capturarVinculoFormulario,true);
document.addEventListener("click",e=>{
  const editar=e.target?.closest?.("#pagina-contas-pagar [data-ap-editar]");if(editar){editandoId=editar.dataset.apEditar||"";setTimeout(()=>prepararFormulario(editandoId),80)}
  if(e.target?.closest?.("#btnNovaContaPagar")){editandoId="";setTimeout(()=>prepararFormulario(""),80)}
...
window.addEventListener("sig:ready",async()=>{await carregarDados();agendarPreparar()});
window.addEventListener("sig:page",async e=>{if(e.detail?.pagina!=="contas-pagar")return;await carregarDados();agendarPreparar()});
window.addEventListener("sig:data-changed",async e=>{if(e.detail?.modulo!==MODULO)return;if(vinculoPendente)await aplicarVinculoPendente();await carregarDados();agendarPreparar()});
preparar();
